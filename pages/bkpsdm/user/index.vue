<template>
  <card style="min-height: 480px">
    ...
    <template slot="header" class="d-inline">
      ...
      <add-user ref="addUserForm" @loadAsyncDataUser="loadAsyncDataUser">
      </add-user>
    </template>
      ...
      <el-table-column min-width="160" label="Nama Lengkap">
        <template slot-scope="scope">
          <div style="padding: 0px !important">
            <span style="margin-top: -6px; color: black" class="">{{
              scope.row.nama_lengkap
            }}</span
            ><br />
            <span
              style="font-size: 11px; margin-top: -6px; color: #696969"
              class="text-muted"
              >{{ scope.row.nip }}</span
            >
          </div>
    ...
    onMergeLinesUser(data) {
      ...
      data.forEach((item, index) => {
        if (index === 0) {
          ...
          if (
            this.tableDataUser[index].id === this.tableDataUser[index - 1].id
          ) {
